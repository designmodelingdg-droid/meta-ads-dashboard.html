# Secuencia A — Calculadora de Zapatas → Especialización en ACERO

**Para:** todo el que descarga la Calculadora de Zapatas (199 con correo hoy, y
los que entren de ahora en adelante).
**Disparador:** tag `lead-calculadora-zapatas` o entrada al pipeline.
**Duración:** 14 días · 7 correos.
**Destino:** Especialización en ACERO.

**Sale de la secuencia** si: responde · agenda · compra · pide baja.

---

## Los datos del producto (confirmados el 11-ago-2026)

Del PDF oficial del programa. **Nada de esto está estimado.**

| | |
|---|---|
| **Nombre completo** | Especialización en **Diseño Estructural BIM en Acero para Edificaciones** |
| **Duración** | 4 meses · 110–130 horas de contenido |
| **Modalidad** | **100 % asincrónica** — a tu ritmo |
| **Precio regular** | $499,99 USD |
| **Precio oferta** | **$199,99 USD** |
| **Vigencia** | *"por tiempo limitado o hasta llenar todos los cupos disponibles"* |
| **Enlace de compra** | `https://designmodelingacademy.com/es/especializacion/diseno-estructural-bim-acero` |

**Los 4 cursos que la componen:**
1. Análisis y Diseño Simplificado de Estructuras de Acero — **Robot Structural Analysis**
2. Guía Práctica para el Cálculo Tipo Cerchas en Naves Industriales — **Robot**
3. Teoría y Cálculo de Uniones Metálicas en Edificaciones — **Advance Steel**
4. Modelado BIM en Hormigón Armado y Acero Estructural — **Revit**

**Lo que se lleva:** 4 certificaciones Autodesk · aval de 120 horas académicas ·
Certificado Universitario Internacional **DQ** (avalado en Europa) · certificado
Modeling DG con **código QR de verificación** · acceso permanente · soporte
técnico de instalación y licencias · comunidad y descuentos.

### La urgencia: cupos, no fecha

El programa **abre todos los meses**, así que no hay fecha de cierre. La urgencia
va por **cupos**, que es lo que dice el material oficial. La regla:

> **Nunca poner un número de cupos que no sea real.** Se dice "cupos limitados"
> y "hasta llenar los cupos disponibles" —que es lo que dice la oferta— o se
> dice el número exacto que Dayana confirme. Un contador falso se descubre solo
> cuando la persona vuelve la semana siguiente y sigue igual.

### Los enlaces

Dominio canónico confirmado por Dayana: **`designmodelingacademy.com`**.

| Para qué | URL |
|---|---|
| **Página del programa** (la de todos los correos) | `designmodelingacademy.com/es/especializacion/diseno-estructural-bim-acero` |
| Checkout directo | `designmodelingacademy.com/checkout/c/S688d24e3d66a2/single` |
| WhatsApp de la oferta | `wa.link/x7qg7c` |

**Verificado el 11-ago contra la página en vivo:** $499,99 → $199,99, *"60 % de
descuento en tu compra, un solo pago de $199.99 USD por 4 formaciones
especializadas"* y *"acceso inmediato y por tiempo ilimitado"*. Cuadra con el
PDF.

> **Nota de método, para no repetir el error:** en la primera comprobación esta
> página devolvió `302 → google.com` y llegué a darla por rota. Era falso: el
> sitio responde así a peticiones **sin cabecera de navegador**. Con un
> `User-Agent` normal responde 200. **Regla: antes de declarar una URL caída,
> repetir la petición con `User-Agent` de navegador.**

### Testimonios — resueltos

Los archivos `.mp4` que pasó Dayana son de formato historia y un correo no
reproduce video. Los **testimonios escritos que sí aparecen en la página** (los
que se ven bajo cada video) son los que van en el correo 6. Están transcritos
literalmente de la página, sin retocar.

---

## Correo 1 · Día 0 — La entrega

> **Esto hoy no existe.** La persona descarga y no recibe nada. Este correo es
> el que más se va a abrir de toda la secuencia: llega cuando acaba de pedirlo.

**Asunto:** Aquí está tu calculadora de zapatas 🎁
**Preview:** Cargas, acero, verificaciones y esquema. Desde el celular.

```
Hola {{contact.first_name}},

Aquí la tienes. Guarda este correo: el enlace no caduca y funciona
igual desde el celular que desde la computadora.

La calculadora hace en 2 minutos los 6 pasos que a mano toman media hora:

✅ Cargas y esfuerzo admisible del suelo
✅ Dimensiones sugeridas en planta (T × S) y altura h
✅ Verificación de cortante y punzonamiento, en vivo
✅ Acero: varillas, diámetro y espaciamiento en las dos direcciones
✅ Esquema de planta y corte, dibujado solo
```

**Botón:** `Abrir mi calculadora` → `https://funnel.dgdesignmodeling.com/calculadora-zapatas`

```
Es la misma metodología de nuestro curso de Diseño de Cimentaciones.

Una aclaración importante y honesta: es una herramienta de
PREDIMENSIONAMIENTO. Te da el punto de partida correcto en segundos, pero
el diseño definitivo lo revisa y lo firma un ingeniero responsable. No
está hecha para saltarse ese paso, sino para que llegues a él con el
problema ya planteado.
```

**PD:** `¿Se te ocurre otra herramienta que te ahorraría tiempo? Responde a este correo y dime cuál — varias de las que tenemos publicadas salieron de una respuesta así.`

---

## Correo 2 · Día 2 — Valor puro, cero venta

> Aquí no se vende nada. Este correo existe para que el tercero y el cuarto se
> abran. Cierra con **pregunta directa** — es la regla que en tres semanas
> seguidas de medición fue lo único que generó conversación (ver
> `calendario-2026-08.md`).

**Asunto:** El error #1 al dimensionar una zapata (y no es el acero)
**Preview:** Lo he visto en planos firmados.

```
Hola {{contact.first_name}},

Cuando reviso predimensionamientos, el error que más veo no está en el
acero ni en las dimensiones. Está un paso antes:

Se toma el esfuerzo admisible del suelo de un estudio de otro proyecto.
O peor: de "lo que se suele usar por aquí".

Y ahí ya nada de lo que venga después importa. Puedes verificar
punzonamiento con tres decimales — si el q_adm está mal, la zapata está
mal. Es el dato que manda sobre todos los demás y es el único que no
puedes calcular tú.

Los otros dos que se repiten:

2️⃣ Verificar cortante y olvidar punzonamiento. El punzonamiento es el que
   manda en zapatas de columnas cargadas, y es el que falla feo.

3️⃣ Dimensionar en planta y dejar la altura "a ojo". La altura no es
   estética: es la que decide si pasas o no pasas las dos verificaciones.

La calculadora te avisa de los dos últimos en vivo. Del primero no puede
avisarte nadie: ese lo traes tú del estudio de suelos.
```

**Botón:** `Volver a la calculadora` → `https://funnel.dgdesignmodeling.com/calculadora-zapatas`

**PD:** `¿Cuál de los tres te ha pasado? Responde y cuéntame — leo todas las respuestas.`

---

## Correo 3 · Día 4 — El puente

> Reencuadre. Todavía no se nombra el producto. Se cambia la pregunta que la
> persona se está haciendo: de "¿está bien mi zapata?" a "¿está bien lo que
> le llega a mi zapata?".

**Asunto:** La zapata casi nunca es el problema
**Preview:** Es el último eslabón de una cadena que empezó arriba.

```
Hola {{contact.first_name}},

Algo que tardé en entender:

La zapata es el ÚLTIMO eslabón. Todo lo que llega a ella —la carga, el
momento, la excentricidad— se decidió arriba, en la estructura.

Si la estructura de arriba está sobredimensionada, la cimentación hereda
ese sobrecosto entero y multiplicado. Si el modelo repartió momentos que
las conexiones reales no pueden tomar, la cimentación recibe unas cargas
que no son las de la obra.

Por eso una zapata "bien calculada" sobre una estructura mal planteada
sigue siendo una zapata cara y mal planteada.

Y es justo donde más plata se pierde sin que nadie lo note: nadie audita
una cimentación que "pasó" las verificaciones.
```

**Botón:** `Ver todas nuestras herramientas gratis` → `https://funnel.dgdesignmodeling.com/recursos`

**PD:** `En el próximo correo te cuento qué hicimos con esto — porque es exactamente el problema que resolvimos para nuestros alumnos de estructuras.`

---

## Correo 4 · Día 7 — La oferta

> Primera vez que se nombra el producto. Con precio (decisión de Dayana:
> lista propia con consentimiento, y ACERO es decisión rápida).

**Asunto:** Lo que va arriba de la zapata (y cómo se aprende)
**Preview:** 4 meses, a tu ritmo, $199,99 en vez de $499,99.

```
Hola {{contact.first_name}},

Te vengo diciendo que la cimentación hereda los errores de arriba. Esto
es lo que hacemos con eso.

La Especialización en Diseño Estructural BIM en Acero para Edificaciones
son 4 cursos que cubren la estructura metálica completa, del
planteamiento al detalle que se fabrica:

🔩 Análisis y diseño de estructuras de acero — Robot Structural Analysis
🔩 Cálculo de cerchas para naves industriales — Robot
🔩 Teoría y cálculo de uniones metálicas — Advance Steel
🔩 Modelado BIM en hormigón armado y acero — Revit

4 meses · 110 a 130 horas · 100% asincrónico, avanzas a tu ritmo.

Y no te llevas solo el conocimiento:

✅ 4 certificaciones Autodesk
✅ Aval de 120 horas académicas
✅ Certificado Universitario Internacional DQ, avalado en Europa
✅ Certificado con código QR de verificación
✅ Acceso permanente al material — no caduca
✅ Soporte técnico para instalar el software y activar licencias

Precio regular: $499,99 USD
Ahora: $199,99 USD

La oferta está vigente hasta llenar los cupos disponibles. El programa
abre todos los meses, así que no es una fecha lo que la cierra — son
los cupos.
```

**Botón:** `Ver el programa y reservar mi cupo` → `https://designmodelingacademy.com/es/especializacion/diseno-estructural-bim-acero`

**PD:** `Si prefieres que te lo cuente una persona antes de decidir, agenda 30 minutos aquí: https://api.leadconnectorhq.com/widget/booking/bIVuNHNojGEgH3gf6yXe`

**→ Quien clique el botón se etiqueta `lead-caliente` y la oportunidad pasa a "Interesado".**

---

## Correo 5 · Día 9 — La objeción real

> La objeción no es el precio, es el tiempo. Se responde de frente y con
> números concretos, sin adornar.

**Asunto:** "Me interesa, pero no tengo tiempo"
**Preview:** Es la respuesta que más recibo. Va en serio.

```
Hola {{contact.first_name}},

Es lo que más me responden, y tiene toda la razón de ser: quien está
diseñando estructuras está ocupado diseñando estructuras.

Así que te lo pongo claro, sin adornos:

📅 NO hay horario. Es 100% asincrónico. No hay clase a la que llegar
   tarde ni sesión que se te pase.
⏱️ Son 110 a 130 horas repartidas en 4 meses. Si lo llevas al ritmo
   sugerido son unas 7 u 8 horas por semana — pero el ritmo lo pones tú.
♾️ El acceso es permanente. Si un mes se te complica el trabajo, lo
   retomas después. No se cierra.
🛟 Y si el problema es instalar el software o activar licencias, eso lo
   resolvemos nosotros. No pierdes un fin de semana en eso.

No te voy a decir que es fácil ni que no cuesta tiempo. Cuesta. Lo que
te digo es que ese tiempo lo recuperas: el rato que hoy se te va en
rehacer un detalle que no coincidía con el modelo es más que lo que te
pide el programa.
```

**Botón:** `Ver cómo funciona` → `https://designmodelingacademy.com/es/especializacion/diseno-estructural-bim-acero`

**PD:** `¿Tu duda es otra? Responde y te contesto yo. No es un correo automático sin salida — al responder me llega a mí.`

---

## Correo 6 · Día 12 — Prueba social

**Asunto:** Lo que dicen los que ya pasaron por aquí
**Preview:** Los alumnos, y las tres preguntas que más me hacen.

```
Hola {{contact.first_name}},

Prefiero que lo escuches de ellos y no de mí:

⭐⭐⭐⭐⭐
"Una institución que te brinda todas las herramientas para crecer
profesionalmente."
— Edy Díaz

⭐⭐⭐⭐⭐
"Una inversión clave para mantenerse vigente en la ingeniería moderna."
— Luis Fernández

⭐⭐⭐⭐⭐
"La mejor decisión que he tomado en mi formación profesional."
— Albino Piñeiro

Están en video en nuestra página de testimonios, por si quieres verlos
y oírlos completos.

Te aviso de una cosa por honestidad: son de distintos programas
nuestros, no todos de la Especialización en Acero. Cuando tenga los de
acero grabados te los mando.

Y estas son las tres preguntas que más me hacen:

❓ ¿Necesito saber modelar antes?
   Base de estructuras sí. Del software nos encargamos nosotros — el
   programa arranca desde los fundamentos BIM aplicados al acero.

❓ ¿Sirve si trabajo con hormigón y no con acero?
   Sirve, y bastante. De hecho uno de los 4 cursos es de modelado BIM
   en hormigón armado Y acero. Y en cuanto te toque una nave, un
   entrepiso metálico o un refuerzo, lo agradeces.

❓ ¿El certificado sirve de algo o es un PDF más?
   4 certificaciones Autodesk, aval de 120 horas académicas, y un
   Certificado Universitario Internacional DQ avalado en Europa. El
   nuestro lleva código QR de verificación, para que quien te contrate
   pueda comprobarlo.
```

**Botón:** `Ver los testimonios` → `https://funnel.dgdesignmodeling.com/testimonio-estudiantes-egresados-diplomado`

**PD:** `¿Tienes una pregunta que no está en la lista? Respóndeme y te contesto yo.`

> ### Sobre los testimonios
>
> Las tres citas están **transcritas literalmente** de la página de testimonios
> (`funnel.dgdesignmodeling.com/testimonio-estudiantes-egresados-diplomado`),
> confirmadas con captura el 11-ago-2026. No se retocaron ni se les cambió una
> palabra.
>
> **Van escritas dentro del correo, no como video**, porque ningún cliente de
> correo reproduce video. El enlace a la página queda para quien quiera verlos
> y oírlos completos — que es exactamente para lo que sirve esa página.
>
> **Lo que falta:** las tres son de alumnos de otros programas, no de la
> Especialización en Acero. El correo lo dice de frente en vez de disimularlo,
> y eso suma en vez de restar. Cuando haya testimonios de acero, se cambian
> estos tres y se quita la frase.
>
> **Regla:** lo que va entre comillas es lo que la persona dijo. Ni una palabra
> más.

---

## Correo 7 · Día 14 — El cierre

**Asunto:** Cierro el tema (y te dejo la puerta abierta)
**Preview:** Último correo de esta serie.

```
Hola {{contact.first_name}},

Este es el último correo de esta serie — de aquí en adelante solo te
llega el contenido de siempre, sin insistir con esto.

Antes de cerrarlo, te lo digo tal cual es: la Especialización en Acero
abre todos los meses, así que no te voy a inventar una fecha límite que
no existe. Lo que sí es cierto es que los cupos de cada apertura son
limitados, y el precio de $199,99 (en vez de $499,99) está atado a que
queden cupos disponibles.

Si lo estabas pensando, es mejor decidirlo ahora que en la próxima
apertura. Y si no es tu momento, no pasa nada: la calculadora sigue
siendo tuya y aquí estaré.

Y si lo que quieres es hablarlo con alguien antes —qué necesitas, si te
sirve, por dónde empezar— agenda 30 minutos. Sin compromiso y sin
discurso de ventas: te digo honestamente si te conviene o no.
```

**Botón:** `Agendar 30 minutos` → `https://api.leadconnectorhq.com/widget/booking/bIVuNHNojGEgH3gf6yXe`

**Segundo enlace, en texto bajo el botón:**
`¿Ya lo tienes decidido? Reserva tu cupo aquí → https://designmodelingacademy.com/es/especializacion/diseno-estructural-bim-acero`

**PD:** `Gracias por leer hasta aquí. Si en algún momento te sirve algo de lo que hacemos, sabes dónde encontrarme.`

---

## La regla de la urgencia por cupos

El programa abre todos los meses. Por eso **no se usa fecha**, se usa cupo. Y
hay una sola regla, que es la que evita que esto se caiga:

- ✅ **"Hasta llenar los cupos disponibles"** — es literalmente lo que dice el
  material oficial de la oferta.
- ✅ **"Los cupos de cada apertura son limitados"** — cierto y verificable.
- ✅ Un número **real** que Dayana confirme ("quedan 8 cupos").
- ❌ **Nunca un contador falso** ni "quedan 3" si no es verdad. Esta gente
  vuelve al mes siguiente, ve lo mismo, y ahí se pierde la credibilidad — que
  es justo lo único que vende un programa de $199,99 por internet.
