# Recomendaciones — Design Modeling Academy

**Este es el tablero.** Todo lo que se recomienda en las revisiones semanales
queda aquí, con fecha, para poder mirar atrás y ver si funcionó.

**Quién escribe aquí:** los skills `matriz-semanal` (revisión de contenido),
`auditoria-pauta` (reporte de la agencia, viernes) y `seguimiento-leads`
(secuencias de correo, mensual). Escriben solos al terminar — no hay que
acordarse de anotar nada.

**Cómo se lee:** lo de arriba es lo más reciente. Cada recomendación tiene
estado: 🔴 pendiente · 🟡 en curso · 🟢 hecho · ⚫ descartada (con el motivo).

---

## 🔴 Abierto ahora mismo

| # | Recomendación | De dónde salió | Quién | Estado |
|---|---|---|---|---|
| 1 | **Pagar los $104,96 pendientes en PayPal.** La cuenta de Meta está en `account_status: 3` (impago) y las campañas están paradas desde el 7-ago con "Error en el pago" | Auditoría 8-ago | Dayana | 🟡 confirmado que se paga |
| 2 | **Bajar presupuesto en Guatemala y subirlo en Ecuador — sin sacar Guatemala.** Guatemala compra más que Ecuador; el costo por conversación no es el criterio | Auditoría 8-ago | Agencia | 🔴 |
| 3 | **Montar las 3 secuencias de correo** de `seguimiento/MONTAJE-GHL.md`. Hoy 2.300 leads no reciben ningún tratamiento | Estrategia 10-ago | Dayana / Claude navegador | 🔴 |
| 4 | **Pedirle a la agencia el costo por VENTA por país**, no por conversación. Es el dato que falta para decidir presupuesto por geografía con fundamento | Auditoría 8-ago | Agencia | 🔴 |
| 5 | **Definir el segundo lead magnet del Mié 13-ago** (checklist de coordinación, plantilla Revit, otra calculadora) | Calendario agosto | Dayana | 🔴 |
| 6 | **Verificar que la membresía se otorga sola** en un registro real del test. Nunca se ha visto el trigger dispararse por sí mismo | Lanzamiento test | Dayana | 🔴 |
| 7 | **Imagen propia para el artículo de la calculadora** en el hub. El archivo que venía con ese nombre resultó ser de otra pieza | Hub de recursos | Diseño | 🔴 |
| 8 | 🚨 **El post del 4-ago promete 3 funciones de IA en Revit que no existen** como se describen. Autodesk: *"hasta Revit 2026 no hay funciones de IA"*. Publicar el artículo del blog ya escrito, responder los comentarios con el enlace y corregir el guion en la matriz | Verificación 11-ago | Dayana / contenido | 🔴 |
| 9 | ~~Confirmar la URL de ACERO~~ · Confirmada: `designmodelingacademy.com/es/especializacion/diseno-estructural-bim-acero`. Verificada en vivo, precio y oferta cuadran con el PDF | Datos ACERO 11-ago | — | 🟢 |
| 10 | ~~Testimonios usables en correo~~ · Resuelto: las 3 citas escritas de la página (Edy Díaz, Luis Fernández, Albino Piñeiro) van dentro del correo 6 | Datos ACERO 11-ago | — | 🟢 |
| 11 | **Grabar testimonios de alumnos de ACERO.** Los tres que hay son de otros programas; el correo lo dice de frente, pero uno de acero convertiría mucho más | Secuencia A 11-ago | Dayana | 🔴 |
| 12 | **Las historias no tienen ni un slot de venta** (0 de 7 días) y los 15 «mándales DM» de la guía no tienen ni un guion escrito. Resuelto en `HISTORIAS-VENTA.md`: jueves de venta + 5 guiones de DM | Auditoría historias 14-ago | Dayana | 🔴 |
| 13 | **ACERO es solo el 20 % de las historias** (3 de 15) siendo el producto de mejor CPL. Subir a 6 | Auditoría historias 14-ago | Dayana | 🔴 |
| 14 | **Decidir si ACERO abre cohorte este mes** y con cuántos cupos reales. Sin ese dato, la secuencia de cuenta regresiva no se publica | Auditoría historias 14-ago | Dayana | 🔴 |

---

## Reglas permanentes (de datos, no de opinión)

Estas ya no se discuten cada semana. Salieron de medir.

### Contenido

| Regla | La evidencia |
|---|---|
| **Escena al abrir + pregunta al cerrar.** Sin pregunta directa no hay comentarios | 3 semanas seguidas. El post del lanzamiento hizo 1.456 views (¼ del Civil 3D) y **21 comentarios contra 0** |
| **El humor va en REEL, nunca en post plano** | El meme estático se quedó en 1.321 views; el humor en video pasa de 4.700 (y el de gremio hizo 8.373) |
| **Humor + pregunta van juntos.** Uno sin el otro da alcance sin conversación | El reel de 8.373 views cerró sin pregunta: 3 comentarios. El del 25-jun que sí preguntaba: 623 |
| **No reciclar el titular del anuncio como hook del post** | La pieza del 9-ago lo hizo: 809 views, 0 comentarios. Quien ya vio el anuncio no encuentra nada nuevo |
| **Lo institucional va a historias, no a feed** | «Somos el equipo detrás»: 2.164 views, 7 likes, 0 guardados |
| **Las piezas de evento no llevan CTA de producto** y se miden por alcance, no por comentarios | Civil 3D: 5.469 views, 0 comentarios, 0 guardados. Es presencia |
| **Facebook es canal propio, no espejo.** Cada pieza fuerte se publica cuidada en FB | 9.103 de las 11.193 views del ganador vinieron de Facebook |

### Pauta

| Regla | Por qué |
|---|---|
| **Ninguna cifra del reporte se da por buena sin verificarla contra Meta** | El 3-ago *todas* las cifras de volumen estaban cortas y el alcance mal por factor 4 |
| **Frecuencia por debajo de 1 es imposible.** Es la señal más rápida de que el dato no salió de Meta | Reportaron 0,86 |
| **Frecuencia arriba de 3 = desgaste.** Se avisa aunque el reporte no lo mencione | — |
| **El costo por conversación NO decide el presupuesto por país** | Ecuador es el más barato y el que menos compra. Falta el costo por venta |
| **Costo por lead barato no es buena noticia solo.** Hay que mirar si las oportunidades se quedan abiertas | — |
| **Las reservas de $100 del Máster no son ventas.** Se separan siempre | — |

### Producto y mensajes

| Regla | Por qué |
|---|---|
| **El Máster nunca se cotiza por contenido ni por chat.** El precio se dice en la llamada | Regla permanente (`CLAUDE.md`). En correo propio sí puede ir el precio de las **Especializaciones**, no el del Máster |
| **Nunca prometer entrega por WhatsApp** en un lead magnet | No existe ese workflow. Se entrega por enlace |
| **Una herramienta gratis no es certificación ni diploma.** Se dice explícito | — |
| **Nunca inventar métricas.** Lo que no viene del dato se marca `s/d` | — |
| 🆕 **Un «mándale DM» sin guion escrito no se ejecuta.** Si una estrategia depende de un mensaje manual, ese mensaje va escrito | La guía de historias dice «les mandas DM» 15 veces y no trae ninguno. Es el mismo hueco que tenían los lead magnets: la conversión no estaba rota, estaba sin construir |
| 🆕 **Toda afirmación técnica sobre una función de software se verifica contra la documentación oficial antes de publicarse** | El carrusel del 4-ago prometía 3 funciones de IA en Revit: una no existe, otra es la mitad falsa y la tercera no funciona como se decía. Vendemos formación técnica — la credibilidad *es* el producto, y eso se lo desmonta cualquier ingeniero en los comentarios |
| 🆕 **La urgencia se construye con lo que es verdad.** Si no hay fecha de cierre, se usan cupos; y nunca un número inventado | ACERO abre todos los meses. Un contador falso se descubre solo cuando la persona vuelve la semana siguiente |

---

## Historial

### 14-ago-2026 · Auditoría de las historias
Dayana señaló que las historias no venden. La verdad es mixta: el diseño **sí**
enruta la venta al DM y eso está bien, pero **la semana tipo no tiene ni un slot
de venta** (0 de 7, y el sábado lo prohíbe expresamente), **ACERO es solo el
20 %** de las secuencias siendo el producto de mejor CPL, y los **15 «mándales
DM» no tienen un solo guion escrito**. Se crea `HISTORIAS-VENTA.md` con el
jueves de venta, 3 secuencias y los 5 guiones de DM. → recomendaciones #12-14.

### 10-ago-2026 · Estrategia de seguimiento de leads
Se detectó que **2.300 contactos de lead magnets no reciben ningún tratamiento**:
no hay correo de bienvenida ni secuencia de nutrición. La "secuencia de
nutrición normal" que mencionaba el brief del bot nunca se escribió.
**Se crean 3 secuencias automáticas** (Zapatas→ACERO, Test→Máster, reactivación
de 2.000 dormidos) en `matriz-viral/seguimiento/`. → recomendación #3.

### 10-ago-2026 · Revisión semanal (3-9 ago)
La pieza con **menos** alcance generó **toda** la conversación de la semana, y
estaba escrita a propósito para probar la lección del 27-jul. Dos reglas nuevas:
piezas de evento sin CTA de producto, y no reciclar el titular del anuncio.

### 8-ago-2026 · Auditoría del reporte (1-7 ago)
Las 13 cifras del reporte **cuadraron con Meta** y se aplicaron las 4
correcciones pedidas el 3-ago. Pero el reporte recomendaba escalar a
$600-800/mes **con la cuenta parada por impago** desde el 7-ago.
→ recomendaciones #1, #2, #4.

### 20-ago-2026 · Auditoría semanal (7 días)
Los **4 cambios pedidos el 3-ago están aplicados**: geo-split abierto (12
adsets), métricas de retorno, conciliación con el CRM y la definición de lead
correcta. 🟢 El gasto cuadra: $442,32 reportado vs $440,17 real (0,5%).

**Retorno real 2,84x** contra el 2,49x reportado — la agencia estaba cerca.
De los $2.249,98 cobrados, **$1.600 son recurrencia del Máster** (3 cuotas de
$160 más $1.120 de dos alumnos que liquidaron su saldo) y $200 son reservas.
Venta nueva: $449,98 por pasarela más $799,99 que solo está en el CRM
—Diplomado y Paquete Autodesk, entrados por transferencia—.

**Lección de método:** ninguna fuente lo ve todo. La pasarela no ve
transferencias; el CRM no ve lo que nadie registra; y las recurrencias inflan
la pasarela si no se separan.

🔴 **Perfilar los leads del Máster.** Es la prioridad que fijó Dayana. Hoy el
adset de Máster FORM trae leads a $1,86 y el de WhatsApp a $1,17, pero sin
filtro de perfil. El Máster no se cierra por volumen.

🔴 **México:** $136,78 y 237 leads con **cero** compras, contra Ecuador con
$143,30, 202 leads y **2** compras. Los adsets mexicanos tienen el CPL más bajo
de la cuenta ($0,32) — el CPL barato es justo lo que engaña. Vigilar dos
semanas antes de mover presupuesto.

🔴 **Cuadrar el segundo ACERO:** la pasarela muestra dos cobros de $199,99 y el
CRM registra uno.

⚫ «Tráfico al perfil» suma $25,50 más por 1 lead. Tercera auditoría sin generar
negocio.

### 3-ago-2026 · Primera auditoría del reporte
*Todas* las cifras de volumen estaban cortas: el reporte se generaba antes de
cerrar el último día ($11 reportado vs $32,67 real). Gasto $231,38 real vs $210
reportado. Alcance 90.115 real vs ~23K reportado. Frecuencia 0,86 (imposible).
Meta ya tenía las métricas de retorno (8 compras, $3.999,99, ROAS 17,3x) y no
se incluyeron. Se pidieron 4 cambios. 🟢 Los 4 se aplicaron para el 8-ago.

### 28-jul-2026 · Ajuste del calendario de agosto
El lead magnet se llevó el 70 % de las vistas y el 74 % de las interacciones de
la semana de prueba. Sube de 1 a 3 apariciones en el mes. Se suman 3 piezas de
**ACERO**, el producto de mejor CPL ($0,45–$0,61) que el orgánico casi no
cubría.

---

## Cómo se mantiene

1. Los skills semanales **añaden una entrada en "Historial"** y **mueven filas**
   de la tabla de arriba cuando algo se hace o se descarta.
2. Nada se borra. Lo descartado queda con ⚫ y el motivo — así no se vuelve a
   proponer lo mismo dentro de tres meses.
3. Si una recomendación lleva **3 semanas en 🔴**, el skill la señala en su
   resumen. O se hace, o se descarta con motivo.
